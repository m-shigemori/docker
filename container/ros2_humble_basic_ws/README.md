...
`ros2_humble_basic_ws`コンテナは以下の環境で構築されます．
...
また，コンテナ内の `colcon_ws/src` は， `ros2_humble_basic_ws/src` と接続されています．(run.shを参照) 
...
1. コンテナのフォルダをコピします．
```bash
$ cp -r {docker_wsのPATH}/container/ros2_humble_basic_ws/ {コンテナPATH}
